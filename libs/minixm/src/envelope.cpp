                            ...
                            unsigned char loop_end_index, unsigned char sustain_index, bool key_off)
{
    if (envelope.count > 0)
    {
        if ((flags & XMEnvelopeFlagsLoop) && position_ == envelope.envelope[loop_end_index].position)
        // if we are at the correct tick for the position
        {
            // loop
        ...
        int current_index = 0;
        while (current_index + 1 < envelope.count && position_ > envelope.envelope[current_index + 1].position)
            current_index++;

        // interpolate new envelope position

        value_ = envelope.envelope[current_index].value + envelope.envelope[current_index].delta * static_cast<float>(
            position_ - envelope.envelope[current_index].position);
